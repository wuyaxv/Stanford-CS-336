...
class BPETokenizer(Tokenizer):
    ...
    def _pop_the_largest_pair(self) -> Tuple[int, int]:
        ...
        while True:
            ...
            if p in self.pair_freq and self.pair_freq[p] == poped_value.count: # 必须确保弹出的pair和当前的pair_freq匹配，否则说明数据过时
                return p
    def __pop_the_largest_pair(self) -> Tuple[int, int]:
        maxed = max(self.pair_freq.items(), key=lambda x: (x[1], x[0]))
        return maxed[0]
    ...
    def _train(self, input_path: str, vocab_size: int, special_tokens: List[str]):
        ...
        _tokens = self._read_mp(input_path)
        assert sorted(_tokens.items())[8:] == sorted(self.word_freq.items())[12:]
        exit(0)
    ...
    def _read_mp(self, input_path):
        PAT = r"""'(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+"""
        tokens = defaultdict(int)
        with open(input_path, 'rb') as f:
            boundaries = self.find_chunk_boundaries(f, mp.cpu_count(), "<|endoftext|>".encode('utf-8'))
            chunk_data = [(start, end, str(input_path)) for start, end in zip(boundaries[:-1], boundaries[1:])]

            process_chunk_partial = partial(self.process_chunk, special_tokens = self.special_tokens, PAT=PAT)
    ...
    def process_chunk(self, chunk_data, special_tokens, PAT):
